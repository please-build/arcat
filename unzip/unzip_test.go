...
func TestExtractEdgeCasesTar(t *testing.T) {
	assert.NoError(t, Extract("unzip/test_data/edgecases.tar", ".", "", ""))
...
func TestExtractEdgeCasesZip(t *testing.T) {
	assert.NoError(t, Extract("unzip/test_data/edgecases.zip", ".", "", ""))
	for _, file := range edgecasesFiles {
		_, err := os.Stat(file)
		assert.NoError(t, err)
	}
...
func TestExtractXMLRunnerTar(t *testing.T) {
	assert.NoError(t, Extract("unzip/test_data/xmlrunner.tar", ".", "", ""))
...
func TestExtractXMLRunnerZip(t *testing.T) {
	assert.NoError(t, Extract("unzip/test_data/xmlrunner.whl", ".", "", ""))
	for _, file := range xmlrunnerFiles {
		_, err := os.Stat(file)
		assert.NoError(t, err)
	}
...
func TestPrefix(t *testing.T) {
	...
	assert.NoError(t, Extract("unzip/test_data/xmlrunner.whl", ".", "", prefix))
	for _, file := range xmlrunnerFiles {
...
func TestSpecificFile(t *testing.T) {
	assert.NoError(t, Extract("unzip/test_data/xmlrunner.whl", "wibble.py", "third_party/python/xmlrunner/result.py", ""))
